...
import { SELF_PACKAGE, fetchGithubLatest, readSelfManifest, selfLayout, selfRepo } from './selfcheck.js'
...
/** pnpm 同理。dsh 的 profile 是 pnpm 管理的，更新插件自身时必须用它。 */
...
/** 诊断日志目录（在 `$DSH_HOME` 下）。只保留最近若干份，免得越攒越多。 */
const LOG_DIRNAME = 'dsh-git-update-notifier-logs'
const MAX_DIAGNOSTIC_FILES = 20

/** 一段输出最多留这么多字符 —— 够检修，又不至于把界面和状态文件撑爆。 */
const MAX_DIAGNOSTIC_CHARS = 128 * 1024

function logDir() {
  return join(homeDir(), LOG_DIRNAME)
}

/** 超出保留份数的诊断日志按修改时间从旧到新删掉。 */
function pruneDiagnostics(dir, say) {
  try {
    const entries = readdirSync(dir)
      .filter((item) => item.startsWith('update-') && item.endsWith('.log'))
      .map((item) => {
    ...
    for (const stale of entries.slice(MAX_DIAGNOSTIC_FILES)) {
      try {
    ...
    if (typeof say === 'function') say('warn', `清理旧诊断日志失败：${describe(error)}`)
  }
}

/** 截断长文本，并说明被截断了多少。 */
function clip(text, max = MAX_DIAGNOSTIC_CHARS) {
  const value = String(text ?? '')
  if (value.length <= max) return value
  return `${value.slice(0, max)}\n…（输出过长已截断，共 ${value.length} 字符）`
}

/**
 * 把一次失败的完整现场写到磁盘，供用户事后检修。
 *
 * 落盘的理由：命令的退出码、完整 stdout / stderr、当时的形态与路径，这些在 UI 上一行
 * 显示不下，而"检修"恰恰需要它们。写日志失败**绝不影响**更新流程本身，只降级成一条告警。
 */
function writeDiagnostic(entry, say) {
  try {
    const dir = logDir()
    mkdirSync(dir, { recursive: true })
    const stamp = new Date().toISOString().replace(/[:.]/g, '-')
    const file = join(dir, `update-${stamp}-${entry.phase ?? 'unknown'}.log`)
    const lines = [
      '# dsh-git-update-notifier 更新失败诊断',
      `时间：${entry.at ?? new Date().toISOString()}`,
      `阶段：${entry.phase ?? '未知'}`,
      `插件版本：${CODE_VERSION}`,
      `安装形态：${entry.layout ?? '未知'}`,
      `包位置：${entry.packageDir ?? '未知'}`,
      entry.gitRoot === undefined ? null : `仓库根：${entry.gitRoot}`,
      entry.projectRoot === undefined ? null : `安装位置：${entry.projectRoot}`,
      `本地版本：${entry.from ?? '未知'}`,
      `目标版本：${entry.to ?? '未知'}`,
      `代理：${entry.proxy ?? '（未使用）'}`,
      `运行时：node ${process.version} / ${process.platform} ${process.arch}`,
      entry.command === undefined ? null : `命令：${entry.command}`,
      entry.exitCode === undefined ? null : `退出码：${entry.exitCode}`,
      `错误：${entry.message ?? ''}`,
      entry.hint === undefined ? null : `建议：${entry.hint}`,
      '',
      '## stdout',
      clip(entry.stdout ?? '', 256 * 1024) || '（空）',
      '',
      '## stderr',
      clip(entry.stderr ?? '', 256 * 1024) || '（空）',
      '',
    ].filter((line) => line !== null)
    writeFileSync(file, lines.join('\n'), 'utf8')
    pruneDiagnostics(dir, say)
    return file
  } catch (error) {
    if (typeof say === 'function') say('warn', `写入诊断日志失败：${describe(error)}`)
    return null
  }
}

/** 读一份诊断日志。只允许读日志目录里的文件，避免被当成任意文件读取的入口。 */
function readDiagnosticFile(file) {
  try {
    ...
    return clip(readFileSync(target, 'utf8'), 256 * 1024)
  } catch {
    return undefined
  }
}
...
/**
 * 给一次失败留证据：写诊断文件，并返回可以放进结果 / 状态里的摘要。
 *
 * 摘要里带一小段 stdout / stderr，界面可以直接显示；完整内容在诊断文件里，
 * 也能通过 `GET /diagnostics.json` 取回。
 */
function diagnose(entry, log) {
  const file = writeDiagnostic(entry, log)
  return {
    file,
    ...
    stdout: clip(entry.stdout ?? '', 32 * 1024),
    stderr: clip(entry.stderr ?? '', 32 * 1024),
  }
}
...
/** 运行 pnpm；约束与 runNpm 相同（Windows 需 shell，参数先过白名单）。 */
function runPnpm(args, cwd, proxy) {
  const shellSafe = process.platform === 'win32' && args.every(npmArgSafe)
  return runCommand(PNPM_COMMAND, args, {
    cwd,
    proxy,
    timeoutMs: NPM_TIMEOUT_MS,
    shell: shellSafe,
 ...
 * 判断某个目录该交给哪个包管理器。
 *
 * dsh 的 profile 是 **pnpm** 管的（`pnpm-workspace.yaml` / `pnpm-lock.yaml` + `.pnpm`），
 * 在那里执行 `npm install` 会打乱它的 `node_modules` 结构与 lockfile —— 而"更新插件自身"
 * 恰好就落在 profile 里，所以这一步必须先认清楚。没有 pnpm 元数据时按 npm 处理
 * （npx 缓存、普通 npm 项目都属于这一档）。
 */
export function packageManagerFor(dir) {
  if (typeof dir !== 'string' || dir === '') return 'npm'
 ...
 */
async function applyGitUpdate(layout, { log, proxy, mark, retry }) {
  const root = layout.gitRoot
  const branchInfo = await currentBranch(root)
  if (!branchInfo.ok) return { ok: false, phase: 'merge', message: branchInfo.message }
  const { branch } = branchInfo

  if (retry === true) {
    // 重试复用上次抓取的结果：FETCH_HEAD 已经指向远端那份提交，不必再过一次网络。
    const hasFetch = await runGit(['rev-parse', '--verify', '--quiet', 'FETCH_HEAD'], root)
    if (hasFetch.code !== 0) {
      return {
        ok: false,
        phase: 'fetch',
        message: '没有可复用的抓取结果（FETCH_HEAD 不存在）',
        hint: '请用「立即更新」重新抓取一次。',
      }
    }
    log('info', '重试：复用上次抓取到本地的 FETCH_HEAD，不重复下载')
    if (typeof mark === 'function') {
      mark({ phase: 'applying', message: '正在重试快进合并（复用已抓取的对象）' })
    }
  } else {
    if (typeof mark === 'function') {
    ...
    const fetched = await fetchUpstream(root, branch, log, proxy)
    if (!fetched.ok) {
      const advice = '网络恢复后再点一次「立即更新」即可 —— git 会复用已下载的对象，不会从头重来。'
      return {
        ok: false,
        phase: 'fetch',
        ...
        diagnostic: diagnose({
          ...
          from: layout.version,
          to: 'origin/' + branch,
          proxy: fetched.attemptedProxy ?? proxy ?? '（未使用）',
          command: `git fetch --quiet --no-tags origin ${branch}`,
          message: `git fetch 失败（${describeRoute(fetched.attemptedProxy)}）`,
          hint: advice,
  ...
  if (merge.code !== 0) {
    const advice = '本地可能有未提交改动或分支已分叉。可以「重试安装」再试一次，或「回退到更新前」放弃这次更新。'
    const status = await runGit(['status', '--short'], root)
    return {
      ok: false,
      phase: 'merge',
      message: `git merge --ff-only 失败（退出码 ${merge.code}）：${tail(merge.stderr)}`,
      ...
      diagnostic: diagnose({
        ...
        exitCode: merge.code,
        message: 'git merge --ff-only 失败：无法快进',
        hint: advice,
        // 顺带记下工作区状态：检修分叉 / 未提交改动时最需要它。
        stdout: `git status --short：\n${status.stdout.trim() || '（工作区干净）'}`,
        stderr: merge.stderr,
      }, log),
    }
  }
 ...
 */
async function applyPackageUpdate(layout, { log, proxy, mark, signal, retry, staged }) {
  // 重试：上一轮已经把包下好并校验过，这次只重做安装这一步。
  if (retry === true && staged !== undefined && staged !== null) {
    log('info', `重试安装 ${staged.version}（复用已校验的本地包，不重新下载）`)
    return installStagedPackage(layout, staged, { log, proxy, mark })
  }

  const channel = channelName()
  const fetched = await fetchDistTags(PACKAGE_NAME, proxy)
  if (!fetched.ok) {
    return { ok: false, phase: 'resolve', message: `查询 npm registry 失败：${fetched.error}` }
  }
  const target = typeof fetched.tags[channel] === 'string' ? fetched.tags[channel] : undefined
  if (target === undefined) {
    return { ok: false, phase: 'resolve', message: `registry 上没有 ${channel} 通道` }
  }
  if (isNewer(target, layout.version) !== true) {
    return { ok: false, phase: 'resolve', message: `本地已是 ${channel} 通道最新（${layout.version}）` }
  }
  ...
  if (!downloaded.ok) {
    ...
    return {
      ...
      diagnostic: diagnose({
        ...
        to: target,
        proxy: proxy ?? '（未使用）',
        message: `下载 ${dist.tarball} 失败：${downloaded.error}`,
        stdout: `URL：${dist.tarball}\n已下载：${kept.bytes} 字节${kept.total === null ? '' : ` / ${kept.total}`}`,
        stderr: String(downloaded.error),
      }, log),
    }
  }
    ...
    if (!verified.ok) {
      ...
      return {
        ...
        diagnostic: diagnose({
          ...
          proxy: proxy ?? '（未使用）',
          message: verified.message,
          stdout: (verified.checks ?? [])
            .map((item) => `${item.ok === true ? '✓' : '✗'} ${item.label}${item.detail === null ? '' : `（${item.detail}）`}`)
            .join('\n'),
        }, log),
      }
    }
  ...
  const installed = await installStagedPackage(layout, prepared, { log, proxy, mark })
  if (installed.ok !== true) {
    // 关键：把"已就绪的包"一起带回去，UI 才能只重试安装、而不重下重校验。
    return { ...installed, staged: prepared, phase: 'install' }
  }
  return installed
}
 ...
 * 安装一个已就绪（且已校验）的包。
 *
 * 独立成函数是因为**安装失败是可重试的**：下载与校验的成果没必要重来一遍。
 * 每次安装前会再算一次哈希，确保"装的就是当初校验通过的那份字节" —— 这既是重试的
 * 安全前提，也让"重试"和"重新下载"在语义上分得干净。
 */
async function installStagedPackage(layout, staged, { log, proxy, mark }) {
  const report = (phase, message, extra) => {
    if (typeof mark === 'function') mark({ phase, message, ...extra })
  }
  const target = staged.version
  const cwd = layout.global ? undefined : layout.projectRoot

  // 安装前重新校验：包可能在这期间被改动，或上一轮校验之后磁盘出过问题。
  let checks = Array.isArray(staged.checks) ? staged.checks : []
  if (staged.installFromFile === true) {
    report('verifying', '安装前重新校验本地包', { version: target })
    let recheck
    try {
      recheck = await verifyTarball(staged.file, {
        integrity: staged.integrity ?? undefined,
        shasum: staged.shasum ?? undefined,
        name: PACKAGE_NAME,
        version: target,
      })
    } catch (error) {
      recheck = { ok: false, message: `校验过程出错：${describe(error)}`, checks: [] }
    }
    if (!recheck.ok) {
      clearPartial(staged.file)
      rmSync(staged.file, { force: true })
      return {
        ok: false,
        message: `本地安装包已不可信：${recheck.message}`,
        hint: '已丢弃这个包，请用「立即更新」重新下载。',
  ...
  const args = staged.installFromFile === true
    ? ['install', staged.file]
    : ['install', `${PACKAGE_NAME}@${target}`]
  if (layout.global) args.push('-g')
  // 项目形态不能把 `file:` 写进用户的 package.json / lock —— 就地换掉 node_modules 即可。
  if (!layout.global && staged.installFromFile === true) args.push('--no-save', '--no-package-lock')
  ...
  if (result.code !== 0) {
    const command = `${NPM_COMMAND} ${args.join(' ')}`
    const advice = staged.installFromFile === true
      ? '安装包已校验通过并留在本地，可以直接「重试安装」（不会重新下载）。'
      : '可改用 DSH_GIT_UPDATE_NOTIFIER_REGISTRY 指向更快的镜像后重试。'
    return {
      ok: false,
      message: `${NPM_COMMAND} install 失败（退出码 ${result.code}）：${tail(result.stderr)}`,
      ...
      diagnostic: diagnose({
        ...
        message: `${NPM_COMMAND} install 失败（退出码 ${result.code}）`,
        hint: advice,
  ...
  const resumeNote = staged.resumedFrom > 0 ? `，其中续传 ${formatBytes(staged.resumedFrom)}` : ''
  const verifyNote = staged.installFromFile === true ? '，校验通过' : ''
  return {
    ok: true,
    message: `已更新到 ${installed}（${where}${verifyNote}${resumeNote}）`,
    output: tail(`${result.stdout}\n${result.stderr}`, 12),
    checks,
    download: staged.installFromFile === true
      ? {
        version: target,
        bytes: staged.bytes,
        total: staged.total,
        resumedFrom: staged.resumedFrom,
      }
      : null,
 ...
 * 检查**本插件自己**有没有新版。
 *
 * 与 dsh 本体的检测对称，但多一层"来源判断"：形态决定去哪儿问版本，而 GitHub 始终兜底 ——
 * 手工复制进 `node_modules` 的那份副本，除了 GitHub 无处可问。
 */
async function inspectSelf(options = {}) {
  const log = options.log ?? (() => {})
  const proxy = 'proxy' in options ? options.proxy : await detectProxy(log)
  const layout = selfLayout()
  const manifest = readSelfManifest()
  const repo = selfRepo(manifest) ?? null
  const base = {
    layout: layout.kind,
    name: typeof layout.name === 'string' && layout.name !== '' ? layout.name : SELF_PACKAGE,
    localVersion: layout.version ?? null,
    packageDir: layout.packageDir,
    profileRoot: layout.profileRoot ?? null,
    repo,
    checkedAt: new Date().toISOString(),
  }
  ...
  if (layout.kind === 'unknown') {
    return {
      ...
      remoteVersion: null,
      canAutoUpdate: false,
      message: '读不到插件自己的 package.json，无法判断当前版本',
      hint: null,
    }
  }

  if (layout.kind === 'checkout') {
    // 源码检出：与 dsh 本体的源码形态走同一条路（fetch + 读 FETCH_HEAD 里的版本号）。
    const branchInfo = await currentBranch(layout.gitRoot)
    if (!branchInfo.ok) {
      return {
        ...base,
        status: 'error',
        ...
        remoteVersion: null,
        canAutoUpdate: true,
        message: `读取插件仓库分支失败：${branchInfo.message}`,
        hint: null,
      }
    }
    const fetched = await fetchUpstream(layout.gitRoot, branchInfo.branch, log, proxy)
    if (!fetched.ok) {
      return {
        ...base,
        status: 'error',
        ...
        remoteVersion: null,
        canAutoUpdate: true,
        message: `插件仓库 git fetch 失败：${tail(fetched.stderr)}`,
        hint: '网络恢复后重新检测即可。',
        viaProxy: fetched.attemptedProxy !== undefined,
      }
    }
    const version = await readVersionAtRef(layout.gitRoot, layout.packageDir, 'FETCH_HEAD')
    if (version === undefined) {
      return {
        ...base,
        status: 'unknown',
        ...
        remoteVersion: null,
        canAutoUpdate: true,
        message: '插件上游那份 package.json 里没有版本号',
        hint: null,
      }
    }
    return finishSelf(base, layout, {
      version,
      source: 'git',
      viaProxy: fetched.proxy !== undefined,
    })
  ...
  if (layout.kind === 'npm') {
    const tags = await fetchDistTags(base.name, proxy)
    const channel = channelName()
    const target = tags.ok && typeof tags.tags[channel] === 'string' ? tags.tags[channel] : undefined
    if (target !== undefined) {
      return finishSelf(base, layout, { version: target, source: 'npm', viaProxy: tags.viaProxy === true })
    }
    // registry 上还没有这个包（尚未发布）时，静默退回下面的 GitHub 兜底。
  }
  ...
  if (repo === null) {
    return {
      ...
      source: null,
      remoteVersion: null,
      canAutoUpdate: layout.kind !== 'manual',
      message: '包清单里没有 GitHub 仓库地址，且不是 git 检出，无法判断远端版本',
      hint: null,
    }
  }

  const github = await fetchGithubLatest(repo, proxy)
  if (!github.ok) {
    return {
      ...base,
      status: 'error',
      ...
      remoteVersion: null,
      canAutoUpdate: layout.kind !== 'manual',
      message: `查询 GitHub 失败：${github.error}`,
      hint: '可用 DSH_GIT_UPDATE_NOTIFIER_PROXY 指定代理后重试；未认证的 GitHub API 有每小时 60 次的限额。',
      viaProxy: github.viaProxy === true,
    }
  }
...
/** 把"远端版本 + 本地版本"折算成最终状态。 */
function finishSelf(base, layout, remote) {
  const newer = isNewer(remote.version, base.localVersion)
  const canAutoUpdate = layout.kind === 'checkout' || layout.kind === 'npm' || layout.kind === 'github'
  const status = newer === undefined ? 'unknown' : newer ? 'update-available' : 'up-to-date'
  const hint = newer === true && layout.kind === 'manual'
    ? '这份副本是手工放进 node_modules 的，没有可用于更新的声明；'
      + `可重新执行 dsh plugin add github:${base.repo ?? 'lmr233/dsh-git-update-notifier'}，或手动替换该目录下的文件。`
    : null
  return {
    ...base,
    status,
    ...
    viaProxy: remote.viaProxy === true,
    canAutoUpdate,
    // 自我更新会替换正在运行的插件代码，必须重启才生效。
    needsRestart: true,
    message: newer === undefined
      ? `无法比较版本：本地 ${base.localVersion ?? '未知'} 与远端 ${remote.version}`
      : null,
    hint,
  }
}
 ...
 * 更新本插件自己。
 *
 * 三种能自动更新的形态各走各的路；`manual` 形态明确拒绝并给出手动做法 ——
 * "不知道这份副本从哪来"时，最安全的动作是不动它。
 */
async function applySelfUpdate(self, { log, proxy, mark }) {
  const report = (phase, message) => {
    if (typeof mark === 'function') mark({ phase, message, version: self.remoteVersion })
  }

  if (self.layout === 'manual' || self.layout === 'unknown') {
    return {
      ok: false,
      message: '这份插件不是通过 git 或包管理器安装的副本，无法就地自动更新',
      hint: `可重新执行 dsh plugin add github:${self.repo ?? 'lmr233/dsh-git-update-notifier'}，`
        + `或手动把新版本文件放到 ${self.packageDir}。`,
    }
  }
  ...
  if (self.layout === 'checkout') {
    const layout = selfLayout()
    if (layout.kind !== 'checkout') {
      return { ok: false, message: '插件目录已不再是 git 检出，无法更新' }
    }
    report('fetching', '正在抓取插件上游（中断后重试会自动续传）')
    const branchInfo = await currentBranch(layout.gitRoot)
    if (!branchInfo.ok) return { ok: false, message: branchInfo.message }
    const fetched = await fetchUpstream(layout.gitRoot, branchInfo.branch, log, proxy)
    if (!fetched.ok) {
      return { ok: false, message: `git fetch 失败：${tail(fetched.stderr)}`, hint: '网络恢复后重试即可。' }
    }
    report('applying', '正在快进合并插件源码')
    const merge = await runGit(['merge', '--ff-only', 'FETCH_HEAD'], layout.gitRoot, proxy)
    if (merge.code !== 0) {
      return {
        ok: false,
        message: `git merge --ff-only 失败：${tail(merge.stderr)}`,
        hint: '本地有未提交改动或分支已分叉，请手动处理后重试。',
      }
    }
    return {
      ok: true,
      message: `插件源码已更新到 ${self.remoteVersion} —— 宿主端代码要**重启 dsh web** 才生效。`,
      needsRestart: true,
    }
  }

  const spec = self.layout === 'github'
    ? `github:${self.repo}${typeof self.remoteVersion === 'string' ? `#v${self.remoteVersion}` : ''}`
    : `${self.name}@${self.remoteVersion}`
  const cwd = typeof self.profileRoot === 'string' && self.profileRoot !== '' ? self.profileRoot : undefined
  if (cwd === undefined) {
    return { ok: false, message: '未能确定插件所在的项目根目录，无法就地更新' }
  }

  // 关键：profile 归 pnpm 管，用错包管理器会破坏它的 node_modules / lockfile。
  const manager = packageManagerFor(cwd)
  const tool = manager === 'pnpm' ? PNPM_COMMAND : NPM_COMMAND
  const args = [manager === 'pnpm' ? 'add' : 'install', spec]
  const label = `${tool} ${args.join(' ')}`

  report('installing', `正在安装 ${spec}（经 ${manager}）`)
  log('info', `执行 ${label}（于 ${cwd}）`)
  const result = manager === 'pnpm'
    ? await runPnpm(args, cwd, proxy)
    : await runNpm(args, cwd, proxy)
  if (result.code !== 0) {
    const advice = manager === 'pnpm'
      ? `可手动执行：cd "${cwd}" && ${label}`
        + `；若被 minimumReleaseAge 拦下，改用 dsh plugin --profile <profile> add ${spec}`
      : `可手动执行：cd "${cwd}" && ${label}`
    return {
      ok: false,
      message: `${label} 失败（退出码 ${result.code}）：${tail(result.stderr)}`,
      output: tail(`${result.stdout}\n${result.stderr}`, 12),
      hint: advice,
      diagnostic: diagnose({
        phase: 'self-install',
        ...
        projectRoot: cwd,
        from: self.localVersion,
        to: self.remoteVersion,
        proxy: proxy ?? '（未使用）',
        command: label,
        exitCode: result.code,
        message: '安装插件新版本失败',
  ...
  return {
    ok: true,
    message: `插件已更新到 ${self.remoteVersion}（经 ${manager}）—— 宿主端代码要**重启 dsh web** 才生效。`,
    output: tail(`${result.stdout}\n${result.stderr}`, 12),
    needsRestart: true,
  }
}
...
export function apply(ctx) {
  ...
  /** 最近一次失败的完整现场（含完整 stdout / stderr），供 `/diagnostics.json` 取用。 */
  let lastDiagnostic = null

  /** 本插件自己的更新状态；与 dsh 本体的检查结果互不影响。 */
  let selfState = null
  ...
  const snapshot = () => {
    ...
    return {
      ...
      // 安装阶段失败后"已就绪、可重试"的包，以及上一次更新的结论。
      stagedPackage: merged.stagedPackage ?? null,
      lastUpdate: merged.lastUpdate ?? null,
      // 插件自身的更新状态（另一条独立的检查线）。
      selfUpdate: selfState,
  ...
  const doDailyCheck = async (reason) => {
    ...
    // 顺带查一次"插件自己"有没有新版。它失败只记日志，绝不影响 dsh 本体的结论。
    try {
      selfState = await inspectSelf({ log: say })
      if (selfState.status === 'update-available') {
        say('warn', `插件自身有新版：${selfState.localVersion ?? '?'} → ${selfState.remoteVersion}`
          + `（来源 ${selfState.source}），可在设置页更新。`)
      } else if (selfState.status === 'up-to-date') {
        say('info', `插件自身已是最新（${selfState.localVersion}）`)
      }
    } catch (error) {
      selfState = {
        status: 'error',
      ...
      say('warn', `检查插件自身更新失败：${describe(error)}`)
    }
  ...
  // 最近一次失败的完整现场（完整 stdout / stderr、命令、退出码），供用户复制去检修。
  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
    path: `${ROUTE_PREFIX}/diagnostics.json`,
    handler: (req, res) => {
      if (req.method !== 'GET') {
        sendJson(res, 405, { code: 'method-not-allowed', message: 'GET only' })
        return
      ...
      const state = readState()
      const summary = (lastResult ?? state).lastUpdate?.diagnostic ?? null
      const file = summary === null || summary === undefined ? null : summary.file
      // 进程内保留着刚失败时的完整输出；重启之后靠磁盘上的诊断文件。
      const content = typeof file === 'string' && file !== '' ? readDiagnosticFile(file) : undefined
      sendJson(res, 200, {
        ok: true,
        latest: lastDiagnostic,
        summary,
        file,
        content: content ?? null,
      })
    },
  }), `${name}: GET diagnostics.json`)

  // 更新插件自己：按安装来源分流（git 检出 / npm 依赖 / github 依赖）。
  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
    path: `${ROUTE_PREFIX}/plugin/update`,
    handler: async (req, res) => {
      ...
      if (selfState === null || selfState.status !== 'update-available') {
        sendJson(res, 409, { code: 'no-update', message: '插件当前没有可更新的新版本' })
        return
      }
      if (selfState.canAutoUpdate !== true) {
        sendJson(res, 409, {
          code: 'manual-only',
          message: '这份插件不是通过 git 或包管理器安装的副本，无法就地自动更新',
          hint: selfState.hint ?? null,
        })
        return
      }

      say('info', `用户确认更新插件自身：${selfState.localVersion ?? '?'} → ${selfState.remoteVersion}`)
      updateProgress = {
        phase: 'starting',
        message: '正在准备更新插件',
        version: selfState.remoteVersion,
        at: new Date().toISOString(),
      }
      let result
      try {
        result = await applySelfUpdate({ ...selfState }, { log: say, mark: markProgress })
      } catch (error) {
        result = { ok: false, message: describe(error) }
      } finally {
        updateProgress = null
      }
      if (result.diagnostic !== undefined && result.diagnostic !== null) lastDiagnostic = result.diagnostic
      if (result.ok === true) {
        selfState = {
          ...selfState,
          status: 'up-to-date',
          localVersion: selfState.remoteVersion,
          checkedAt: new Date().toISOString(),
          message: null,
          hint: null,
        }
  ...
  /** 取"待安装的包"：先看本进程内存，再回退到状态文件（重启后仍可重试）。 */
  const stagedPackageFromState = () => {
    const fromMemory = (lastResult ?? {}).stagedPackage
    if (fromMemory !== undefined && fromMemory !== null) return fromMemory
    const fromDisk = readState().stagedPackage
    return fromDisk === undefined || fromDisk === null ? undefined : fromDisk
  }

  /**
   * 跑一次更新并落状态，`/update` 与 `/update/retry` 共用。
   *
   * 失败按阶段区分：只有**安装阶段**（`npm install` / `git merge --ff-only`）失败才提供
   * "重试安装"与"回退到更新前"。理由很直接 —— 那两个动作的前提分别是"包已就绪"和
   * "工作区可能已经被改动"；下载阶段的失败另有断点续传那条路。
   */
  const runUpdateTask = async (layout, { retry, attempt }) => {
    ...
    const staged = retry ? stagedPackageFromState() : undefined
    updateProgress = {
      phase: retry ? 'installing' : 'starting',
      message: retry ? '正在重试安装' : '正在准备更新',
      at: new Date().toISOString(),
    }
    downloadController = new AbortController()
    pending = applyUpdate(layout, {
      log: say,
      mark: markProgress,
    ...
    if (result.ok && result.after !== undefined) {
      ...
      const phase = typeof result.phase === 'string'
        ? result.phase
        : (result.interrupted === true ? 'download' : null)
      const installFailed = phase === 'install' || phase === 'merge'
      // 安装阶段失败：工作区可能已经动过，所以既要能重试、也要能回退。
      // 回退点在更新前就取好了，这里只是把它正式记下来供 /rollback 使用。
      let rollback = (lastResult ?? {}).rollback ?? null
      if (installFailed) {
        rollback = {
          ...rollbackBase,
          from: rollbackBase.version,
          to: typeof result.staged?.version === 'string' ? result.staged.version : rollbackBase.version,
          at: finishedAt,
          failed: true,
        }
        rollback.script = writeRollbackScript(rollback, say)
      }
      // 完整现场留在内存（供 /diagnostics.json 立即取用），状态文件里只留摘要。
      if (result.diagnostic !== undefined && result.diagnostic !== null) {
        lastDiagnostic = result.diagnostic
        say('warn', `已写入诊断日志：${result.diagnostic.file ?? '（写盘失败，仅内存可查）'}`)
      }
      lastResult = {
        ...(lastResult ?? {}),
        // 下载中断时把断点记进状态 —— 重启后 UI 仍能看到"可继续"。
        pendingDownload: result.resumable === true ? (result.download ?? null) : null,
        stagedPackage: installFailed && result.staged !== undefined ? result.staged : null,
        rollback,
        lastUpdate: {
          ok: false,
          at: finishedAt,
          ...
          phase,
          canRetry: installFailed,
          canRollback: installFailed && rollback !== null,
          needsFreshDownload: result.needsFreshDownload === true,
          attempt,
          diagnostic: result.diagnostic === undefined || result.diagnostic === null
            ? null
            : {
              file: result.diagnostic.file ?? null,
              command: result.diagnostic.command ?? null,
              exitCode: result.diagnostic.exitCode ?? null,
              phase: result.diagnostic.phase ?? null,
              at: result.diagnostic.at ?? finishedAt,
            },
        },
        dismissed: true,
        lastCheckDate: today(),
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      const result = await runUpdateTask(layout, { retry: false, attempt: 1 })
      if (result.ok) say('info', String(result.message))
      else say('warn', String(result.message))
      sendJson(res, result.ok ? 200 : 500, result)
    },
  ...
  // 安装阶段失败后的重试：复用已就绪的包（或已抓取的 FETCH_HEAD），不重新下载。
  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
    path: `${ROUTE_PREFIX}/update/retry`,
    handler: async (req, res) => {
      if (req.method !== 'POST') {
        sendJson(res, 405, { code: 'method-not-allowed', message: 'POST only' })
        return
      ...
      const previous = (lastResult ?? {}).lastUpdate
      if (previous === null || previous === undefined || previous.canRetry !== true) {
        sendJson(res, 409, { code: 'no-retry', message: '没有可重试的安装失败记录' })
        return
      }
      ...
      if (layout.kind !== 'source') {
        const staged = stagedPackageFromState()
        if (staged === undefined) {
          sendJson(res, 409, { code: 'no-package', message: '没有已就绪的安装包，请用「立即更新」重新下载' })
          return
        }
        // 安装位置变了（换了形态或换了目录）就不能拿旧包硬装。
        if (staged.packageDir !== layout.packageDir || (staged.global === true) !== (layout.global === true)) {
          sendJson(res, 409, { code: 'layout-changed', message: '安装位置已变化，请用「立即更新」重新下载' })
          return
        }
      }

      const attempt = (typeof previous.attempt === 'number' ? previous.attempt : 1) + 1
      say('info', `用户重试安装（第 ${attempt} 次，${layoutLabel(layout)}）`)
      const result = await runUpdateTask(layout, { retry: true, attempt })
      if (result.ok) say('info', String(result.message))
      else say('warn', String(result.message))
      sendJson(res, result.ok ? 200 : 500, result)
    },
  }), `${name}: POST update retry`)
