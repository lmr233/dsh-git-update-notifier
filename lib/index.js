...
async function applySelfUpdate(self, { log, proxy, mark }) {
  ...
  report('installing', `正在安装 ${spec}`)
  log('info', `执行 ${NPM_COMMAND} install ${spec}（于 ${cwd}）`)
  const result = await runNpm(['install', spec], cwd, proxy)
  if (result.code !== 0) {
    const advice = `可手动执行：cd "${cwd}" && ${NPM_COMMAND} install ${spec}`
    return {
      ok: false,
      message: `${NPM_COMMAND} install ${spec} 失败（退出码 ${result.code}）：${tail(result.stderr)}`,
      output: tail(`${result.stdout}\n${result.stderr}`, 12),
      hint: advice,
      diagnostic: diagnose({
        phase: 'self-install',
        ...
        proxy: proxy ?? '（未使用）',
        command: `${NPM_COMMAND} install ${spec}`,
        exitCode: result.code,
        message: '安装插件新版本失败',
        hint: advice,
        stdout: result.stdout,
  ...
  return {
    ok: true,
    message: `插件已更新到 ${self.remoteVersion} —— 宿主端代码要**重启 dsh web** 才生效。`,
    output: tail(`${result.stdout}\n${result.stderr}`, 12),
    needsRestart: true,
  }
}
