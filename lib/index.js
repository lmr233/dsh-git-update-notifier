 ...
 * 测试会断言两处常量一致，避免忘记同步。
 */
const CODE_VERSION = '0.2.0-dev.3'
...
export function apply(ctx) {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      const rollback = readState().rollback
      if (rollback === null || rollback === undefined) {
        sendJson(res, 409, { code: 'no-rollback', message: '没有可回退的更新记录' })
        return
      }
      say('info', `用户确认回退到 ${rollback.from}`)
      let result
      try {
        result = await applyRollback(rollback, { log: say })
      } catch (error) {
        result = { ok: false, message: describe(error) }
      }
