    ...
    function UpdateCard() {
      ...
      children.push(h('div', { className: 'gun_head', key: 'head' },
        h('span', { className: 'gun_title' },
          isUpdate ? 'DeepSeek Harness 有新的上游提交' : '上游更新检查失败'),
        h('span', { className: 'gun_badge', 'data-error': isError ? 'true' : 'false' },
          isUpdate ? '落后 ' + String(status.behind || 0) + ' 个提交' : '未能检查')))

      children.push(h('div', { className: 'gun_body', key: 'body' },
        isUpdate
          ? h('div', { className: 'gun_kv' }, h('b', null, '分支'), h('span', null, String(status.branch || '-')))
          : null,
        isUpdate
          ? h('div', { className: 'gun_code' }, shortSha(status.localHead) + ' → ' + shortSha(status.remoteHead))
          : h('div', { className: 'gun_code' }, String(status.message || '检查未能完成')),
        status.hint === undefined || status.hint === null ? null : h('div', null, String(status.hint)),
        h('div', { className: 'gun_kv' }, h('b', null, '仓库'), h('span', null, String(status.root || '-')))))

      if (Array.isArray(status.subjects) && status.subjects.length > 0) {
      ...
      if (result !== null) {
        ...
        if (result.ok) {
          text += '。这是源码 checkout，需要重新构建（pnpm build:lib）并重启 dsh 后才会生效。'
        }
