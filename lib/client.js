...
.gun_pre{margin:0;max-height:240px;overflow:auto;padding:8px 10px;border-radius:8px;background:rgba(127,127,127,.09);color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:1.5;white-space:pre-wrap;overflow-wrap:anywhere}
...
/** 与宿主端 CODE_VERSION 必须一致（测试会断言），用于两半自检。 */
    ...
    function UpdateSection() {
      ...
      var snoozeOpen = snoozeState[0]
      var setSnoozeOpen = snoozeState[1]
      // 回退：先列出可选目标（懒加载），选中后再确认 —— 不可逆的写操作。
      var rollbackState = react.useState({ open: false, targets: null, pick: null })
      var rollback = rollbackState[0]
      var setRollback = rollbackState[1]
      // 下载进度：更新是同步等待的（宿主那边一直阻塞到装完），所以靠轮询补充
      // "下载到哪了、是不是在续传"——只有更新进行中才轮询，平时不发请求。
      var progressState = react.useState(null)
      var progress = progressState[0]
      var setProgress = progressState[1]
      // 完整报错：懒加载 —— 只有点了「查看完整报错」才去取，平时不发请求。
      var diagState = react.useState({ open: false, text: null, loading: false })
      var diag = diagState[0]
      var setDiag = diagState[1]

      react.useEffect(function () {
        if (busy !== 'update' && busy !== 'plugin-update') {
          setProgress(null)
          return undefined
        }
        var alive = true
        var tick = function () {
          fetch(ROUTE + '/progress.json', { cache: 'no-store' })
            .then(function (res) { return res.ok ? res.json() : null })
            .then(function (body) { if (alive && body !== null) setProgress(body) })
            .catch(function () { /* 轮询失败不影响更新本身 */ })
        }
        tick()
      ...
      /** 打开回退目标列表：点开才去问宿主，避免每次加载都跑 git log / npm view。 */
      function onOpenRollback() {
        setRollback({ open: true, targets: null, pick: null })
        fetch(ROUTE + '/rollback/targets.json', { cache: 'no-store' })
          .then(function (res) { return res.ok ? res.json() : {} })
          .then(function (body) {
            setRollback({ open: true, targets: Array.isArray(body.targets) ? body.targets : [], pick: null })
          })
          .catch(function () { setRollback({ open: true, targets: [], pick: null }) })
      }

      /** 回退到已选目标；未选目标时不带 target，由宿主用上次更新前的记录。 */
      function onRollback() {
        var target = rollback.pick === null || rollback.pick === undefined ? '' : String(rollback.pick.id)
        setBusy('rollback')
        setNote(null)
        post('/rollback' + (target === '' ? '' : '?target=' + encodeURIComponent(target))).then(function (answered) {
          setBusy(null)
          setRollback({ open: false, targets: null, pick: null })
       ...
       * 更新插件**自己**。
       *
       * 与 dsh 本体的更新是两条独立的线：这条改的是插件的安装（git 检出走快进合并，
       * npm / github 依赖走 npm install）。宿主端换掉之后同样要重启 dsh web。
       */
      function onPluginUpdate() {
        setBusy('plugin-update')
        setNote(null)
        post('/plugin/update').then(function (answered) {
          setBusy(null)
          setNote(String(answered.body.message || ''))
          refresh()
        })
       ...
       * 展开 / 收起完整报错。
       *
       * 宿主那边优先给"刚失败时的完整输出"（进程内保存），重启后回退到磁盘上的诊断文件，
       * 所以这里只管把拿到的内容原样摊开给用户复制。
       */
      function onToggleDiagnostics() {
        if (diag.open === true) {
          setDiag({ open: false, text: null, loading: false })
          return
        }
        setDiag({ open: true, text: null, loading: true })
        fetch(ROUTE + '/diagnostics.json', { cache: 'no-store' })
          .then(function (res) { return res.ok ? res.json() : null })
          .then(function (body) {
            if (body === null) {
              setDiag({ open: true, text: '读取诊断信息失败。', loading: false })
              return
            }
            var parts = []
            var latest = body.latest
            if (latest !== null && latest !== undefined) {
              if (latest.command) parts.push('命令：' + String(latest.command))
              if (typeof latest.exitCode === 'number') parts.push('退出码：' + String(latest.exitCode))
              if (latest.stdout) parts.push('--- stdout ---\n' + String(latest.stdout))
              if (latest.stderr) parts.push('--- stderr ---\n' + String(latest.stderr))
            }
            if (parts.length === 0 && typeof body.content === 'string' && body.content !== '') {
              parts.push(body.content)
            }
            if (parts.length === 0) parts.push('没有可用的诊断内容。')
            setDiag({ open: true, text: parts.join('\n\n'), loading: false })
          })
          .catch(function () { setDiag({ open: true, text: '读取诊断信息失败。', loading: false }) })
      }

      /**
       * 安装阶段失败后的重试：只重做安装这一步。
       *
       * 宿主那边会复用已经下载并校验过的包（源码形态则复用已抓取的 FETCH_HEAD），
       * 所以这次不会再走一遍下载。
       */
      function onRetry() {
        setBusy('retry')
        setNote(null)
        post('/update/retry').then(function (answered) {
          setBusy(null)
          setNote(String(answered.body.message || ''))
          refresh()
        })
      ...
      /**
       * 中断下载。默认保留断点（下次从断点续），discard 为 true 时连断点一起丢。
       *
       * 此刻 /update 还阻塞在宿主那边，收尾交给它自己，这里不把 busy 归零。
       */
      function onCancelDownload(discard) {
        setBusy('cancel')
        post('/download/cancel' + (discard === true ? '?discard=1' : '')).then(function (answered) {
          setNote(String(answered.body.message || (answered.ok ? '已取消下载' : '取消失败')))
        })
      }
      ...
      // 插件自身的更新状态：与 dsh 本体是两条独立的检查线。定义在块外 ——
      // 下面的按钮区在 status 为空时也要能安全引用它。
      var selfInfo = status !== null && status.selfUpdate !== null && status.selfUpdate !== undefined
        ? status.selfUpdate
        : null
      var facts = []
      if (status !== null) {
      ...
      if (status === null) {
        ...
        if (selfInfo !== null) {
          var selfText
          if (selfInfo.status === 'update-available') {
            selfText = '插件自身有新版：' + String(selfInfo.localVersion || '?')
              + ' → ' + String(selfInfo.remoteVersion || '?')
              + '（来源 ' + String(selfInfo.source || '未知') + '）'
          } else if (selfInfo.status === 'up-to-date') {
            selfText = '插件自身已是最新（' + String(selfInfo.localVersion || '?') + '）'
          } else if (selfInfo.status === 'error') {
            selfText = '插件自身检查失败：' + String(selfInfo.message || '')
          } else {
            selfText = '插件自身版本：' + String(selfInfo.localVersion || '未知') + '（无法判断是否有新版）'
          }
          children.push(h('p', { className: 'gun_msg', 'data-note': 'self', key: 'self' }, selfText))
          if (selfInfo.hint) {
            children.push(h('p', { className: 'gun_msg', 'data-note': 'self-hint', key: 'self-hint' },
              String(selfInfo.hint)))
          }
        }
      ...
      // 更新在途：显示阶段、下载进度，以及这次是不是从断点续上的。
      if (busy === 'update' || busy === 'cancel') {
        var live = progress !== null && progress.progress !== null && progress.progress !== undefined
          ? progress.progress
          : null
        var livePending = progress !== null && progress.pendingDownload !== null && progress.pendingDownload !== undefined
          ? progress.pendingDownload
          : null
        var phaseText = live !== null && live.message ? String(live.message) : '正在更新…'
        var liveBytes = live !== null && typeof live.bytes === 'number' ? live.bytes : null
        var liveTotal = live !== null && typeof live.total === 'number'
          ? live.total
          : (livePending !== null && typeof livePending.total === 'number' ? livePending.total : null)
        if (liveBytes !== null && liveBytes > 0) {
          if (liveTotal !== null && liveTotal > 0) {
            var percent = Math.min(100, Math.round((liveBytes / liveTotal) * 100))
            phaseText += ' — ' + formatBytes(liveBytes) + ' / ' + formatBytes(liveTotal) + '（' + String(percent) + '%）'
          } else {
            phaseText += ' — 已下载 ' + formatBytes(liveBytes)
          }
        }
        if (live !== null && typeof live.resumedFrom === 'number' && live.resumedFrom > 0) {
          phaseText += ' · 从断点续传'
        }
        children.push(h('p', { className: 'gun_msg', 'data-note': 'progress', key: 'progress' }, phaseText))
      }

      // 磁盘上留着断点：上次下载被打断了，再点一次「立即更新」会接着下。
      var pendingInfo = status !== null && status.pendingDownload !== null && status.pendingDownload !== undefined
        ? status.pendingDownload
        : null
      if (pendingInfo !== null && busy === null) {
        children.push(h('p', { className: 'gun_msg', 'data-note': 'partial', key: 'partial' },
          '上次下载中断，已保留 ' + formatBytes(pendingInfo.bytes)
          + (typeof pendingInfo.total === 'number' && pendingInfo.total > 0
            ? ' / ' + formatBytes(pendingInfo.total)
            : '')
          + '（版本 ' + String(pendingInfo.version || '-')
          + '）。再次点击「立即更新」会从断点继续，不会从头下载。'))
      }

      // 上次更新留下的包校验结论。
      var lastUpdate = status !== null && status.lastUpdate !== null && status.lastUpdate !== undefined
        ? status.lastUpdate
        : null
      if (lastUpdate !== null && Array.isArray(lastUpdate.checks) && lastUpdate.checks.length > 0) {
        var checkText = lastUpdate.checks.map(function (item) {
          return (item.ok === true ? '✓ ' : '✗ ') + String(item.label)
            + (typeof item.detail === 'string' && item.detail !== '' ? '（' + item.detail + '）' : '')
        }).join('　')
        children.push(h('p', { className: 'gun_msg', 'data-note': 'checks', key: 'checks' },
          '上次更新的包校验：' + checkText))
      }

      // 失败现场：命令、退出码、诊断日志路径（所有阶段的失败都适用，不只安装阶段）。
      var diagSummary = lastUpdate !== null && lastUpdate.ok === false
        && lastUpdate.diagnostic !== null && lastUpdate.diagnostic !== undefined
        ? lastUpdate.diagnostic
        : null
      if (diagSummary !== null) {
        var diagBits = []
        if (diagSummary.command) diagBits.push('命令：' + String(diagSummary.command))
        if (typeof diagSummary.exitCode === 'number') diagBits.push('退出码：' + String(diagSummary.exitCode))
        if (diagSummary.file) diagBits.push('诊断日志：' + String(diagSummary.file))
        if (diagBits.length > 0) {
          children.push(h('p', { className: 'gun_msg', 'data-note': 'diagnostic', key: 'diagnostic' },
            diagBits.join('　')))
        }
      }
      if (diag.open === true) {
        children.push(h('pre', { className: 'gun_pre', key: 'diagnostic-text' },
          diag.loading === true ? '正在读取完整报错…' : String(diag.text || '')))
      }

      // 安装阶段失败：说清"为什么能重试、为什么能回退"，按钮在下面。
      if (lastUpdate !== null && (lastUpdate.canRetry === true || lastUpdate.canRollback === true)) {
        children.push(h('p', { className: 'gun_msg', 'data-note': 'install-failed', key: 'install-failed' },
          '安装阶段失败（第 ' + String(lastUpdate.attempt || 1) + ' 次）：' + String(lastUpdate.message || '')
          + (lastUpdate.needsFreshDownload === true
            ? '　本地安装包已失效，需要重新下载。'
            : '　安装包已校验并留在本地 —— 可以只「重试安装」，也可以「回退到更新前」放弃这次更新。')))
      }
      ...
      // 自选回退目标：常驻入口，点开后列出最近可回去的提交 / 版本。
      if (status !== null) {
        if (rollback.pick !== null && rollback.pick !== undefined) {
          actions.push(h('button', {
            key: 'rollback-yes', className: 'gun_btn', type: 'button', 'data-primary': 'true',
            disabled: busy !== null, onClick: onRollback,
          }, busy === 'rollback'
            ? '回退中…'
            : '确认回退到 ' + String(rollback.pick.label || rollback.pick.id)))
          actions.push(h('button', {
            key: 'rollback-no', className: 'gun_btn', type: 'button',
            disabled: busy !== null,
            onClick: function () { setRollback({ open: true, targets: rollback.targets, pick: null }) },
          }, '取消'))
        } else if (rollback.open === true) {
          var targetList = rollback.targets
          if (targetList === null || targetList === undefined) {
            actions.push(h('button', { key: 'rollback-wait', className: 'gun_btn', type: 'button', disabled: true }, '读取可选目标…'))
          } else if (targetList.length === 0) {
            actions.push(h('button', { key: 'rollback-none', className: 'gun_btn', type: 'button', disabled: true }, '没有可选目标'))
          } else {
            targetList.forEach(function (item) {
              actions.push(h('button', {
                key: 'rollback-' + String(item.id), className: 'gun_btn', type: 'button',
                disabled: busy !== null,
                onClick: function () { setRollback({ open: true, targets: targetList, pick: item }) },
              }, String(item.label || item.id)))
            })
          }
          actions.push(h('button', {
            key: 'rollback-cancel', className: 'gun_btn', type: 'button',
            disabled: busy !== null,
            onClick: function () { setRollback({ open: false, targets: null, pick: null }) },
          }, '取消'))
        } else {
          actions.push(h('button', {
            key: 'rollback', className: 'gun_btn', type: 'button',
            disabled: busy !== null, onClick: onOpenRollback,
          }, '回退…'))
        }
      }
      // 插件自身有新版且能自动更新时，给一个独立的按钮（与 dsh 本体的更新分开）。
      if (selfInfo !== null && selfInfo.status === 'update-available' && selfInfo.canAutoUpdate === true) {
        actions.push(h('button', {
          key: 'plugin-update', className: 'gun_btn', type: 'button',
          disabled: busy !== null, onClick: onPluginUpdate,
        }, busy === 'plugin-update' ? '更新插件中…' : '更新插件'))
      }
      // 任何阶段的失败都留一个"看完整报错"的入口，方便复制去检修。
      if (lastUpdate !== null && lastUpdate.ok === false) {
        actions.push(h('button', {
          key: 'diagnostics', className: 'gun_btn', type: 'button',
          disabled: busy !== null, onClick: onToggleDiagnostics,
        }, diag.open === true ? '收起完整报错' : '查看完整报错'))
      }
      // 安装阶段失败后的两条出路：重试安装（复用已就绪的包）／回退到更新前。
      if (lastUpdate !== null && lastUpdate.canRetry === true) {
        actions.push(h('button', {
          key: 'retry-install', className: 'gun_btn', type: 'button', 'data-primary': 'true',
          disabled: busy !== null, onClick: onRetry,
        }, busy === 'retry' ? '重试中…' : '重试安装'))
      }
      if (lastUpdate !== null && lastUpdate.canRollback === true) {
        actions.push(h('button', {
          key: 'rollback-failed', className: 'gun_btn', type: 'button',
          disabled: busy !== null, onClick: onRollback,
        }, busy === 'rollback' ? '回退中…' : '回退到更新前'))
      }
      // 更新/下载进行中：给一个明确的"中断"出口（默认保留断点，下次能续）。
      if (busy === 'update') {
        actions.push(h('button', {
          key: 'cancel-download', className: 'gun_btn', type: 'button',
          onClick: function () { onCancelDownload(false) },
        }, '中断下载'))
        actions.push(h('button', {
          key: 'discard-download', className: 'gun_btn', type: 'button',
          onClick: function () { onCancelDownload(true) },
        }, '中断并丢弃断点'))
      } else if (busy === 'cancel') {
        actions.push(h('button', {
          key: 'cancel-pending', className: 'gun_btn', type: 'button', disabled: true,
        }, '正在中断…'))
      }
